...
public class FlinkMonitorRunner extends AbstractMonitorRunner {
    ...
    public void executeJob() {
        ...
        if (appInfo != null) {
            ...
            //任务阻塞判断
            if (monitor.getWaitingBatches() == null || monitor.getWaitingBatches() == 0) {
                return;
            }
            //获取背压数据
            ...
            //批次积压超阈值-告警
            if (isOutBatches) {
                LOGGER.info("flink task out of backpressure ratio, scriptId: " + monitor.getScriptId() + ", backpressure ratio(*100)="
                        + backpressure.ratio + ", max ratio(*100)=" + maxBatches);
                //积压重启
                if (monitor.getAutoRestart() != null && monitor.getAutoRestart()) {
                    YarnApiUtils.killApp(cluster.getYarnUrl(), appInfo.getAppId());
                    //防止更新不及时
                    try {
                        Thread.sleep(5000);
            ...
            if (monitor.getExAutoRestart() != null && monitor.getExAutoRestart()) {
                // 重启
                boolean restart = restart();
                if (restart) {
                    if (httpYarnApp != null) {
