!function (win) {
        ...
        if (cate_list) {
            ...
            if (href.indexOf('#') !== -1) {
                ...
                for (var i = 0; i < targets.length; i ++) {
                    target = targets[i];
                    var attr = $(target).attr('bw-target').split('$');
                    if (attr[1] === href) {
                        match = true;
    ...
    Xadmin.prototype.add_tab = function (title, url, multi, is_refresh) {
        ...
        var wins = [];
        //重复点击
        var $xIframe = $('.x-iframe');
        for (var i = 0; i < $xIframe.length; i++) {
            if (multi) {
                if ($xIframe.eq(i).attr('tab-id') && $xIframe.eq(i).attr('tab-id').indexOf(id) !== -1) {
                    wins.push($xIframe.eq(i).attr('tab-id'));
                }
            } else {
                if ($xIframe.eq(i).attr('tab-id') === id) {
                    element.tabChange('xbs_tab', id);
                    if (is_refresh) {
                        $xIframe.eq(i).attr("src", $xIframe.eq(i).attr('src'));
                    }
                    return;
                }
            }
        ...
        if (multi) {
            if (wins.length === 2) {
                element.tabChange('xbs_tab', wins[1]);
                if (is_refresh) {
                    $('[tab-id="' + wins[1] + '"]').attr('src', $('[tab-id="' + wins[1] + '"]').attr('src'));
                }
                return;
            } else {
                if (wins.length) {
    ...
    $('.left-nav #nav').on('click', 'li', function (event) {
        ...
        if ($(this).children('.sub-menu').length) {
            if ($(this).hasClass('open')) {
                ...
                $(this).find('.nav_right').removeClass('fa-angle-down');
                $(this).find('.nav_right').addClass('fa-angle-left');
                $(this).children('.sub-menu').stop(true, true).slideUp();
                $(this).siblings().children('.sub-menu').slideUp();
            } else {
                $(this).addClass('open');
                $(this).siblings().find('.nav_right').removeClass('fa-angle-down');
                $(this).siblings().find('.nav_right').addClass('fa-angle-left');
                $(this).children('.sub-menu').stop(true, true).slideDown();
                $(this).siblings().children('.sub-menu').stop(true, true).slideUp();
                $(this).find('.nav_right').removeClass('fa-angle-left');
                $(this).find('.nav_right').addClass('fa-angle-down');
